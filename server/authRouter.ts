...
import { createSession, hashPassword, requireAuth, requireRoles, verifyPassword } from "./auth.js";
...
router.post("/login", async (req, res, next) => {
  try {
    ...
    if ((user.role === UserRole.SUPPLIER_ADMIN || user.role === UserRole.SUPPLIER_RECRUITER) && !user.supplierId)
...
router.get("/suppliers", requireAuth, async (req, res) => {
  const where = req.auth?.role === UserRole.SUPPLIER_ADMIN || req.auth?.role === UserRole.SUPPLIER_RECRUITER
...
router.post("/users", requireAuth, requireRoles(UserRole.PLATFORM_ADMIN, UserRole.SUPPLIER_ADMIN), async (req, res, next) => {
  try {
    ...
    const supplierRole = body.role === UserRole.SUPPLIER_ADMIN || body.role === UserRole.SUPPLIER_RECRUITER;
    if (req.auth?.role === UserRole.SUPPLIER_ADMIN && body.role !== UserRole.SUPPLIER_RECRUITER)
...
router.put("/users/:id", requireAuth, requireRoles(UserRole.PLATFORM_ADMIN, UserRole.SUPPLIER_ADMIN), async (req, res, next) => {
  try {
    ...
    const target = await prisma.user.findFirst({ where: { id: String(req.params.id), ...(req.auth?.role === UserRole.SUPPLIER_ADMIN ? { supplierId: req.auth.supplierId || "", role: UserRole.SUPPLIER_RECRUITER } : {}) } });
