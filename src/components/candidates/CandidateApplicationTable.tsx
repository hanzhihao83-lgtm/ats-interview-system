...
export default function CandidateApplicationTable({ businessLine, status }: { businessLine?: BusinessLine; status?: string }) {
  ...
  return <>
    <Space className="table-toolbar" wrap><Input allowClear prefix={<SearchOutlined />} placeholder="搜索姓名、应聘编号或手机号后四位" value={keyword} onChange={(e) => setKeyword(e.target.value)} onPressEnter={() => load(1)} /><Button onClick={() => load(1)}>查询</Button>{!user?.simulation && hasPermission("DATA_EXPORT") && <Button icon={<DownloadOutlined />} onClick={() => void download()}>导出 Excel</Button>}</Space>
    <Table rowKey="id" loading={loading} dataSource={rows} pagination={{ current: page, total, pageSize: 20, onChange: load }} scroll={{ x: 1180 }} columns={[
      ...
      { title: "负责人", render: (_, r) => r.owner?.name || "—" },
