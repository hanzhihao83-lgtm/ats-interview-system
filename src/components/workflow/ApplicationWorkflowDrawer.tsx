...
import { calendarApi, type InterviewerProfile } from "../../api/calendarApi";
...
export default function ApplicationWorkflowDrawer({ applicationId, businessLine, onClose, onChanged }: Props) {
  ...
  const [reschedulingInterview, setReschedulingInterview] = useState<WorkflowInterview | null>(null);
  ...
  const writable = !user?.simulation;
  const canScreen = writable && hasPermission("SCREENING_SUBMIT");
  const canSchedule = writable && hasPermission("INTERVIEW_SCHEDULE");
  const canViewFeedback = hasPermission("FEEDBACK_VIEW");
  const canSubmitFeedback = writable && hasPermission("FEEDBACK_SUBMIT");
  const canOffer = writable && hasPermission("OFFER_MANAGE");
  const canLevel = writable && hasPermission("LEVEL_ADJUSTMENT_REQUEST");
  const canOnboard = writable && hasPermission("ONBOARDING_CONFIRM");
  ...
  useEffect(() => {
    if (!scheduleOpen || !detail) return;
    calendarApi.interviewers(detail.businessLine === "UNCLASSIFIED" ? undefined : detail.businessLine)
      .then(setInterviewers)
      .catch((error) => message.error(error instanceof Error ? error.message : "面试官列表加载失败"));
  }, [scheduleOpen, detail]);
  ...
  const submitSchedule = async () => {
    ...
    const start = values.scheduledStartTime as Dayjs;
    const task = reschedulingInterview
      ? workflowApi.rescheduleInterview(reschedulingInterview.id, {
          scheduledStartTime: start.toISOString(),
          scheduledEndTime: start.add(30, "minute").toISOString(),
          interviewerId: values.interviewerId,
          reason: values.reason,
        })
      : workflowApi.scheduleInterview(detail!.id, {
          scheduledStartTime: start.toISOString(),
          scheduledEndTime: start.add(30, "minute").toISOString(),
          round: values.round,
    ...
    const ok = await run(reschedulingInterview ? "面试时间已更新" : "面试已排期并生成会议记录", () => task);
    if (ok) {
  ...
  return <>
      ...
      {loading && !detail ? <div style={{ textAlign: "center", padding: 80 }}><Spin size="large" /></div> : detail ? <Space direction="vertical" size={16} style={{ width: "100%" }}>
        <Card size="small">
          <Descriptions size="small" column={3}>
            ...
            <Descriptions.Item label="联系方式">{detail.candidate.phone || detail.candidate.email || detail.candidate.phoneMasked || detail.candidate.emailMasked || "—"}</Descriptions.Item>
          </Descriptions>
          <Steps size="small" current={currentStep} items={workflowSteps.map((title) => ({ title }))} style={{ marginTop: 20, overflowX: "auto" }} />
          {canScreen && detail.currentStatus === "简历待筛选" && <Space style={{ marginTop: 18 }}>
        ...
        <Card title={<><CalendarOutlined /> 面试排期与面评</>} size="small" extra={
          canSchedule && ["待安排面试", "待面试", "面试待反馈"].includes(detail.currentStatus)
            ? <Space>
              <Button onClick={() => { history.pushState({}, "", "/calendar"); window.dispatchEvent(new PopStateEvent("popstate")); }}>查看时间看板</Button>
              <Button type="primary" onClick={() => { scheduleForm.setFieldsValue({ round: detail.interviews.length + 1, roundName: `第${detail.interviews.length + 1}轮` }); setScheduleOpen(true); }}>安排面试</Button>
            </Space>
            : null
        }>
          {detail.interviews.length ? <List
            ...
            renderItem={(interview) => <List.Item actions={[
              ...((canViewFeedback || canSubmitFeedback) ? [<Button key="feedback" type={interview.feedbackRecord?.status === "SUBMITTED" || !canSubmitFeedback ? "default" : "primary"} onClick={() => openFeedback(interview)}>
                {canSubmitFeedback ? (interview.feedbackRecord?.status === "SUBMITTED" ? "查看/更新面评" : "提交面评") : "查看面评"}
              </Button>] : []),
              ...(canSchedule && !["已取消", "取消"].includes(interview.status) ? [
                <Button key="reschedule" onClick={() => {
                  setReschedulingInterview(interview);
                  scheduleForm.setFieldsValue({ scheduledStartTime: dayjs(interview.scheduledStartTime), interviewerId: interview.interviewerProfileId, round: interview.round, roundName: interview.roundName, reason: "HR 调整面试时间" });
                  setScheduleOpen(true);
                }}>改期</Button>,
                <Button key="cancel" danger onClick={() => Modal.confirm({ title: "确认取消该场面试？", content: "取消原因将记录为“HR 取消面试”，并通知面试官。", okButtonProps: { danger: true }, onOk: () => run("面试已取消", () => workflowApi.cancelInterview(interview.id, "HR 取消面试")) })}>取消</Button>,
              ] : []),
            ]}>
              <List.Item.Meta
        ...
        <Card title={<><GiftOutlined /> Offer 状态机</>} size="small" extra={canOffer && detail.currentStatus === "面试通过" && !["PENDING_INITIATION", "SENT"].includes(latestOffer?.status || "") ? <Button type="primary" onClick={() => void run("Offer 已发起", () => workflowApi.createOffer(detail.id))}>发起 Offer</Button> : null}>
          ...
          {latestOffer && <Space wrap>
            {canOffer && latestOffer.status === "PENDING_INITIATION" && <Button type="primary" icon={<SendOutlined />} onClick={() => void run("Offer 已发出", () => workflowApi.sendOffer(latestOffer.id))}>标记已发出</Button>}
            {canOffer && latestOffer.status === "SENT" && <>
              ...
              <Button onClick={() => void run("Offer 已失效", () => workflowApi.expireOffer(latestOffer.id))}>标记失效</Button>
            </>}
          </Space>}
        </Card>

        <Card title="职级调整审批" size="small" extra={canLevel ? <Button onClick={() => setLevelOpen(true)}>申请调整职级</Button> : null}>
        ...
        <Card title={<><TeamOutlined /> 入职确认与接待闭环</>} size="small" extra={canOnboard && latestOffer?.status === "CANDIDATE_CONFIRMED" && !detail.onboarding ? <Button type="primary" onClick={() => { onboardingForm.setFieldsValue({ result: "CONFIRMED", assigneeName: "入职接待同学" }); setOnboardingOpen(true); }}>确认入职结果</Button> : null}>
          ...
          {detail.receptionTask && <Card type="inner" title={`接待任务 · ${detail.receptionTask.assigneeName || "待指派"}`} extra={<Tag color={detail.receptionTask.status === "COMPLETED" ? "green" : "blue"}>{detail.receptionTask.status}</Tag>} style={{ marginTop: 12 }}>
            ...
            <Space direction="vertical">
              {detail.receptionTask.checklist.map((item) => <Checkbox key={item.id} checked={item.completed} disabled={!isInternal || detail.receptionTask?.status === "COMPLETED" || acting} onChange={(event) => void run("接待清单已更新", () => workflowApi.toggleChecklist(detail.receptionTask!.id, item.id, event.target.checked))}>{item.title}{item.required ? "（必填）" : ""}</Checkbox>)}
            </Space>
            {isInternal && detail.receptionTask.status !== "COMPLETED" && <div style={{ marginTop: 16 }}><Button type="primary" icon={<CheckCircleOutlined />} disabled={detail.receptionTask.checklist.some((item) => item.required && !item.completed)} onClick={() => void run("接待已完成，候选人进入培训中", () => workflowApi.completeReception(detail.receptionTask!.id))}>完成接待并回执</Button></div>}
          </Card>}
        </Card>
    ...
    <Modal open={scheduleOpen} title={reschedulingInterview ? "调整面试时间" : "安排面试"} onCancel={() => { setScheduleOpen(false); setReschedulingInterview(null); }} onOk={() => void submitSchedule()} confirmLoading={acting} destroyOnClose>
      <Form form={scheduleForm} layout="vertical">
        <Alert type="info" showIcon message="面试固定 30 分钟，只可从整点或半点开始；提交时会再次校验全平台冲突、10 分钟缓冲和固定休息时间。" style={{ marginBottom: 16 }} />
        <Form.Item name="scheduledStartTime" label="面试开始时间" rules={[{ required: true, message: "请选择开始时间" }]}><DatePicker showTime={{ minuteStep: 30 }} format="YYYY-MM-DD HH:mm" style={{ width: "100%" }} /></Form.Item>
        <Space align="start" style={{ width: "100%" }}>
          <Form.Item name="round" label="轮次" rules={[{ required: !reschedulingInterview }]}><InputNumber disabled={Boolean(reschedulingInterview)} min={1} precision={0} /></Form.Item>
          <Form.Item name="roundName" label="轮次名称" rules={[{ required: !reschedulingInterview }]}><Input disabled={Boolean(reschedulingInterview)} placeholder="例如：业务复试" /></Form.Item>
        </Space>
        <Form.Item name="interviewerId" label="面试官" rules={[{ required: true, message: "请选择面试官" }]}><Select showSearch optionFilterProp="label" placeholder="选择面试官" options={interviewers.map((profile) => ({ value: profile.id, label: `${profile.user.name}${profile.department ? ` · ${profile.department}` : ""}` }))} /></Form.Item>
        {reschedulingInterview && <Form.Item name="reason" label="改期原因" rules={[{ required: true }, { min: 2 }]}><Input.TextArea rows={3} /></Form.Item>}
      </Form>
    </Modal>

    <Modal open={Boolean(feedbackInterview)} title={`${canSubmitFeedback ? "填写" : "查看"}面评 · ${feedbackInterview?.roundName || "面试"}`} onCancel={() => setFeedbackInterview(null)} onOk={canSubmitFeedback ? () => void submitFeedback() : () => setFeedbackInterview(null)} okText={canSubmitFeedback ? "提交面评" : "关闭"} confirmLoading={acting} destroyOnClose>
      <Alert type="info" showIcon message="所有维度均为必填，面评提交后才允许生成最终结论；结束 24 小时未提交将触发催办。" style={{ marginBottom: 16 }} />
      <Form form={feedbackForm} layout="vertical">
        <Space wrap>{feedbackTemplate.dimensions.map((dimension) => <Form.Item key={dimension} name={dimension} label={dimension} rules={canSubmitFeedback ? [{ required: true, message: "请评分" }] : []}><InputNumber disabled={!canSubmitFeedback} min={1} max={5} precision={0} addonAfter="/ 5" /></Form.Item>)}</Space>
        <Form.Item name="comment" label="面试评语" rules={canSubmitFeedback ? [{ required: true }, { min: 5, message: "评语不少于 5 个字" }] : []}><Input.TextArea disabled={!canSubmitFeedback} rows={4} maxLength={1000} showCount /></Form.Item>
      </Form>
    </Modal>
