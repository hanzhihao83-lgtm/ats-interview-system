...
export default function InterviewBookingButton({ candidate }: { candidate: Candidate }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const meetingResponse = await fetch(`/api/interviews/${created.data.id}/create-meeting`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ startTime, endTime, subject: `招聘面试｜${candidate.name}｜${candidate.position}｜第一轮` }) });
